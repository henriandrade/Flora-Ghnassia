<script lang="ts">
  ...
  function setupAnimation() {
    ...
    const initAnimations = () => {
      ...
      const initTextAnimation = (textElement: HTMLElement) => {
        ...
        const splitInstance = splitInstances.find((instance) => {
          return instance.lines?.some(
            (line) => line.firstChild === textElement
          );
        });
      ...
      // Cleanup function for initAnimations
      return () => {
    ...
    if (smile && prefaceLoader) {
      ...
      // If smile or prefaceLoader doesn't exist, initialize animations directly
      initAnimations();
    }
  }
